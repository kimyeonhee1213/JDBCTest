import java.sql.*;
import java.util.ArrayList;

public class JoinDAO {
    public static String RDB_DRIVE = "com.mysql.cj.jdbc.Driver";
    public static String URL = "jdbc:mysql://localhost/study";
    public static String USER = "root";
    public static String PWD = "";

    /**
     * DB 연결을 하기 위한 메소드
     * DB 연결 정의를 기반으로 DB 에 연결하고 리턴값으로 연결정보를 리턴함
     * @return Connection
     */
    private static Connection getConnection() {
        try {
            Class.forName(RDB_DRIVE);
            Connection conn = DriverManager.getConnection(URL, USER,
                    PWD);
            return conn;
        } catch(Exception e) {
            throw new IllegalStateException(e);
        }
    }
    /**
     * DB 에서 조인한 데이터를 검색하는 메소드
     * student 테이블과 exams 테이블을 조인한 데이터를 ArrayList 에 저장하고 리턴값 리턴
     * @return list
     */

    public ArrayList<JoinDTO> join(){
        Connection conn = null;
        Statement smt = null;
        // 배열선언
        ArrayList<JoinDTO> list = new ArrayList<JoinDTO>();
        String sql = "SELECT * FROM exams JOIN students USING (student_id) GROUP BY student_id";

        try {
            // DB 연결
            conn = JoinDAO.getConnection();
            smt = conn.createStatement();
            // SQL 문 실행
            ResultSet rs = smt.executeQuery(sql);
            // 검색결과 ArrayList 에 저장
            while(rs.next()) {
                JoinDTO objDTO = new JoinDTO();
                objDTO.setStudentId(rs.getString("student_id"));
                objDTO.setStudentName(rs.getString("student_name"));
                objDTO.setExamNo(rs.getInt("exam_no"));
                objDTO.setSubject(rs.getString("subject"));
                objDTO.setScore(rs.getInt("score"));
                list.add(objDTO);
            }
        } catch (SQLException e) {
            System.out.println("오류 발생: " + e);
        } finally {
            // 사용한 리소스 제거
            if(smt != null) {
                try {
                    smt.close();
                } catch (SQLException ignore) {}
                try {
                    conn.close();
                } catch (SQLException ignore) {}
            }
        }
        return list;
    }

}
